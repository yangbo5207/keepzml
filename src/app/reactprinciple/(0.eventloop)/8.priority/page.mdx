...
Linux 内核中对各个进程的调度分配、定时器的实现原理、React Fiber 的任务调度分配等等，都利用了优先级队列的思维来解决问题。因此作为一名合格的高级前端工程师，优先级队列是必须要掌握的重要知识点。

通常情况下，优先级队列都由二叉堆来实现。二叉堆是一颗完全二叉树：即在树结构中，除了最后一层，其他节点都是完整的「每一个节点都拥有左右两个子节点」。它分为小顶堆与大顶堆两种类型。

> 二叉堆中，每一个节点都必须是可比较的，否则就无法判断优先级

大顶堆的特性为：两个子节点的值，始终小于父节点的值。因此，最顶层的父节点是最大的值。

小顶堆的特性为：两个子节点的值，始终大于父节点的值。因此，最顶层的父节点是最小的值。

对于二叉堆，只有几个常规操作：插入节点，删除节点，以及构建二叉堆。这里我们以小顶堆为例进行介绍。

### *1、* 插入节点

二叉堆的节点插入，只能是二叉堆树结构中的最后一个位置。以上面的最小堆为例，我想要插入一个值为 1 的节点。

![](/images/reactprinciple/i1.png)

插入之后，我们发现并不符合小顶堆的规则，这个时候就需要进行「上浮」调整。将插入的子节点，与其父节点进行比较，如果子节点小于父节点，那么按照小顶堆的规则，就必须将父子节点进行互换

![](/images/reactprinciple/i2.png)

节点 1 上浮之后，还需要继续跟其新的父节点进行比较，我们发现它的父节点 3 还是要比 1 大，那么还要继续上浮

![](/images/reactprinciple/i3.png)

发现新的父节点 2 仍然比 1 大，还要继续上浮，最终让 1 成为新的堆顶

![](/images/reactprinciple/i4.png)

### *2、* 删除节点

在二叉堆中，我们通常只会删除处于堆顶的元素。

![](/images/reactprinciple/d1.png)

但是删除之后，我们会发现，二叉堆的结构就出现了混乱，为了维持稳定，并且保证二叉堆的特性，还需要在删除之后进行一些调整，首先需要将树结构中的最后一个节点，补充到堆顶位置

![](/images/reactprinciple/d2.png)

补充之后发现，当前的树结构不符合最小堆的特性，因此需要将新的堆顶元素，与子元素进行比较，找到最小的子元素与其交换位置，这个行为我们称之为「下沉」

![](/images/reactprinciple/d3.png)

继续与新的子元素进行比较，如果仍然比最小的子元素大，则需要继续下沉，直到完全符合最小堆的规则为止

![](/images/reactprinciple/d4.png)
...
除了 sort 方法之外，map, filter, reduce, some, every 等方法，都采用了类似的思维，传入一个条件，根据条件的执行结果，返回新的内容。


### *5、* 总结

优先级队列是 React 底层调度机制的核心算法之一。因此我们一定要彻底掌握。