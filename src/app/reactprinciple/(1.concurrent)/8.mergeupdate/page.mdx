...
只要我们保存了数据，当 diff 更新发生时，我们就可以知道如何处理这些数据。因此，批处理的核心就是，多次保存更新的数据，这些更新数据会组成一个链表，然后一次性统一处理。而我们不需要执行多次 diff 更新。因此到通知根节点开始更新的环节，我们只需要在第一次 `ensureRootIsScheduled` 时，通知根节点有一个更新产生，而后续再同一轮函数调用栈就不需要重复通知了。
